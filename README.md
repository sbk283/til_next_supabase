...
- Zustands(불어 state) 로서 전역 상태관리
- Recoil 과 흡사하지만 Next.js 에서는 React 19 로서 Recoil 지원안함
...
export interface CounterState {
  ...
  increment: () => void; // 카운터 값을 증가시키는 함수
  decrement: () => void; // 카운터 값을 감소시키는 함수
  reset: () => void; // 카운터 값을 초기화하는 함수
  setCount: (value: number) => void; // 카운터 값을 특정 숫자로 설정하는 함수
}
```
